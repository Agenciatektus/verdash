...
function SortableStageCard({ id, children }: { id: string, children: React.ReactNode }) {
...
export const FunnelConfigForm = ({ 
  ...
  onUpdate,
  onCancel
}: FunnelConfigFormProps & { onCancel?: () => void }) => {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active && over && active.id !== over.id) {
      const oldIndex = stages.findIndex((s) => s.step === active.id);
      const newIndex = stages.findIndex((s) => s.step === over.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={stages.map((s) => s.step)} strategy={verticalListSortingStrategy}>
          <div className="space-y-4">
            {stages.map((stage, index) => (
              <SortableStageCard key={stage.step} id={stage.step}>
